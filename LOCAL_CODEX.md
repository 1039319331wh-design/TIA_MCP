...
解释 LAD/FBD 逻辑时优先调用 `tia_get_block_networks`，逐网络结合标题、注释、符号、调用和指令分析。需要评估一个块依赖的外部对象时调用 `tia_get_block_references`；不能把普通指令部件误认为用户程序块。

开始 I/O 编程或地址调整前调用 `tia_audit_plc_io`，检查重复地址、重复符号名、缺失类型和缺失注释。工具只确认完整地址文本相同的冲突；涉及 `%IB/%IW/%ID` 或位地址范围重叠时必须人工结合数据宽度复核。

清理符号或排查未定义引用时调用 `tia_audit_symbol_usage`。只能把结果视为审计线索：HMI、报警、配方、外部系统、间接寻址和未扫描块都可能形成工具看不到的引用，禁止根据“可能未使用”结果自动删除变量。

新建代码块第一阶段仅支持 SCL FB/FC。先调用 `tia_prepare_scl_block`，审核完整源码、声明类型、块名和源码哈希；只有用户明确确认后才调用 `tia_apply_prepared_scl_block` 并传入 `CREATE_SCL_BLOCK`。创建目标固定为 PLC 根程序块组，不得声称支持任意分组；创建后必须确认编译零错误。失败回滚或保存仍需检查工具结果，禁止自动下载到 PLC。
